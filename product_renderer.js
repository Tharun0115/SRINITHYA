...
function createProductCard(product) {
    ...
    const specsHTML = (product.specs || []).map(spec => `
        <li class="flex items-center">
            <i class="${spec.icon || 'fa-solid fa-check'} text-primary w-6"></i>
            <span>${spec.text}</span>
        </li>
    `).join('');
    ...
    const actionsHTML = (product.actions || []).map(action => {
        if (action.type === 'cart') {
            ...
            return `<button onclick="addToCart(${params})" class="w-full bg-secondary text-white py-2 rounded font-bold hover:bg-yellow-600 transition text-sm"><i class="fa-solid fa-plus"></i> Add to Estimate</button>`;
        }
        if (action.type === 'enquire') {
            return `<button class="w-full bg-primary text-white font-bold py-2 rounded hover:bg-blue-800 text-sm transition">Enquire</button>`;
        }
        if (action.type === 'enquire-link') {
            return `<a href="${rootPath}${action.href}" class="w-full bg-secondary text-white py-2 rounded font-bold hover:bg-yellow-600 transition mt-auto"><i class="fa-solid fa-headset"></i> Enquire Now</a>`;
        }
        return '';
    }).join('');
    ...
    const imageContainerClass = product.imageContainerClass || 'h-64';
    ...
    return `
        <div class="${cardWrapperClass}" ${compareDataAttributes}>
            <div class="${imageContainerClass} bg-gray-100 flex items-center justify-center relative overflow-hidden">
                <img onclick="openImageModal('${imageSrc}')" src="${imageSrc}" alt="${product.name}" loading="lazy" decoding="async" class="w-full h-full ${imageClass} transition-transform duration-300 group-hover:scale-110 cursor-pointer">
                ${badgeHTML}
            </div>
            <div class="p-6 text-center flex flex-col flex-grow">
                ...
                <ul class="text-sm text-left text-gray-700 space-y-2 mb-6 inline-block w-full px-4">
                    ${specsHTML}
                </ul>
